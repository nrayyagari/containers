...
## Context & Problem
This topic explains how published ports expose container listeners on host addresses. In production, this matters because most container network incidents are really packet-path or name-resolution problems hidden by abstraction.
Port publishing is not the same thing as application-level service discovery or firewall design.
...
After that, continue to [Network Namespaces](../07-network-namespaces/README.md).
