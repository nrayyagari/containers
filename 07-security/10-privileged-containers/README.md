...
## Context & Problem
This topic explains what privileged mode actually grants and why it is dangerous. In production, this matters because container security fails when teams treat one control as sufficient instead of layering identity, policy, and trust.
Privileged mode is not a minor flag change. It removes large parts of the isolation model.
...
After that, continue to [Security Best Practices](../11-security-best-practices/README.md).
